...
```csharp
// "Pedir un código que no existe debe lanzar NoEncontradoExcepcion"
try
{
...
servicio depende de `IRepositorioProducto`, no de SQL Server**. Es el
...
| Mutante 3 | *(la línea del `throw` borrada)* | se quitó el efecto |
...
| Qué medir | Herramienta (C# / .NET) | Qué entrega |
|---|---|---|
| Cobertura | **coverlet** (`dotnet test /p:CollectCoverage=true`) + ReportGenerator | % por línea y por rama, reporte HTML |
| Complejidad / CRAP | Analizadores de Visual Studio o **SonarQube** | Complejidad ciclomática por método |
| Mutación | **Stryker.NET** (`dotnet stryker`) | Mutantes creados, muertos y sobrevivientes, con reporte HTML |
